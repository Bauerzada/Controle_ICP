...
function getOpsDb_(){const p=PropertiesService.getScriptProperties(),id=p.getProperty(ICP_OPS_DB.property);if(id){try{const f=DriveApp.getFileById(id);if(!f.isTrashed())return SpreadsheetApp.openById(id)}catch(e){}}
 const ss=SpreadsheetApp.create(ICP_OPS_DB.name);const f=DriveApp.getFileById(ss.getId()),folder=DriveApp.getFolderById(ICP_CONFIG.pastaProjetoId);folder.addFile(f);try{DriveApp.getRootFolder().removeFile(f)}catch(e){};p.setProperty(ICP_OPS_DB.property,ss.getId());ensureOpsTabs_(ss);return ss}
